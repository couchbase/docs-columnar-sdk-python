...
Eventually the SDK will be published to PyPI, but in the interim a select set of wheels are available on the https://github.com/couchbaselabs/columnar-python-client/releases[Github Releases page].  If a wheel is not available for your specific Python version and/or platform, see the https://github.com/couchbaselabs/columnar-python-client/blob/main/BUILDING.md[Github Building page] for details on how to build the SDK's binary.
